...
- a staged podcast can be taken back out. **Remove from campaign** deletes the lead at the provider, which stops any sequence step that has not gone yet and returns the pitch to editable; it cannot recall an email already delivered, and the confirmation says so when the campaign was live at the time. Launched outreach is refused here and pointed at pausing the campaign instead;
- a refused send — an opt-out, a duplicate contact, a pitch already read — stays on screen beside the draft it refers to rather than passing as a notification;
...
Relationships is the workspace's durable host CRM, and it is deliberately curated rather than automatic. A show enters the book only when a person puts it there: **Add relationship**, or **Save to relationships** from the Master Inbox. Launching outreach, receiving a reply, recording a booking, and the pre-campaign admin outreach tool all leave the book untouched — otherwise the handful of relationships someone actually filed would be buried under every show the agency has ever emailed. Owners and admins record a stage and summary, link clients, and log notes, calls, or meetings against the hosts they have filed.

Derived outreach state is a separate question and still reads everything: campaigns, inbox threads, bookings, and the legacy ledger. So a host the agency has contacted still triggers the relationship warning before a re-pitch and still blocks a cold open, whether or not anyone filed them in the book.
