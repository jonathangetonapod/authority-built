...
// client whose SDR mode is auto_draft, every new attributed host reply is
// classified and packaged (reply + numbered nudges) into the review queue.
...
serve(async (req) => {
  ...
  for (const client of (autoClients ?? []) as Array<{ id: string; workspace_id: string | null }>) {
  ...
  return json(200, { drafted, prefiltered, skipped, workspaces: (integrations ?? []).length })
})
