-- Two applications share one Resend account: this one on getonapod.com and
-- aisdr.getonapod.com. Resend has no per-domain webhook scope, so both
-- endpoints receive both applications' events, and this function treated an
-- unrecognised email id as a retryable server error. A sibling application's
-- traffic would therefore retry against this endpoint until Resend disabled it
-- -- which is the state the endpoint was found in.
--
-- The sender domain decides ownership, and an unmatched id is only retried for
-- as long as the race it exists for can plausibly still be open.

BEGIN;

DROP FUNCTION IF EXISTS public.process_resend_webhook_event(
...
BEGIN
  ...
  -- Resend delivers every event for the account to every registered endpoint,
  -- with no per-domain scope, so a sibling application sharing this Resend
  -- account reaches this webhook too. Only this application's own sending
  -- domains are ours to record; anything else is receipted and dropped.
  sender_domain := lower(btrim(COALESCE(
    substring(p_from_address FROM '@([^@>[:space:]]+)>?[[:space:]]*$'),
    ''
  ), '>'));

  IF sender_domain <> ALL (ARRAY['mail.getonapod.com']) THEN
    RETURN 'ignored';
  END IF;
...
COMMENT ON FUNCTION public.process_resend_webhook_event(
  ...
  'Service-only atomic Resend receipt scoped to this application''s sending domains, with monotonic delivery update, engagement count, and suppression operation.';
