...
const response: ClientPodcastSystemResponse = {
  ...
  clients: [{ id: clientId, name: 'Taylor Client', status: 'active', photo_url: null, podcast_count: 3 }],
  summary: {
    total: 3,
    active: 2,
    completed: 1,
...
describe('WorkspaceClientPodcastSystem', () => {
  ...
  it('gives a workspace owner a real work queue, pipeline, completed view, and connected detail', async () => {
    renderPage()

    expect(await screen.findByRole('heading', { name: 'Client Podcast System' })).toBeInTheDocument()
    expect(screen.getByText('Private to this workspace')).toBeInTheDocument()
    expect(screen.getByLabelText('Search client podcast opportunities')).toBeInTheDocument()
    expect(screen.getByLabelText('Filter by contact availability')).toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: 'Client review' })).toBeInTheDocument()
    ...
    fireEvent.mouseDown(screen.getByRole('tab', { name: 'Work queue' }), { button: 0 })
    fireEvent.change(screen.getByLabelText('Search client podcast opportunities'), {
  ...
  it('keeps raw contact emails out of the member-safe lifecycle response', async () => {
    ...
    renderPage()
    await screen.findByRole('heading', { name: 'Client Podcast System' })
    fireEvent.change(screen.getByLabelText('Search client podcast opportunities'), {
      target: { value: 'Operator Stories' },
    })
    fireEvent.click(screen.getByRole('button', { name: 'Open' }))
